...
class ChooseRelatedCompanyWidget extends StatefulWidget {
  ChooseRelatedCompanyWidget(
      {Key key, this.selectedItemBloc, this.httpEvent, this.selected})
      : super(key: key);
  ...
  @override
  _ChooseRelatedCompanyWidgetState createState() =>
      _ChooseRelatedCompanyWidgetState(
          this.selectedItemBloc, this.httpEvent, this.selected);
}

class _ChooseRelatedCompanyWidgetState
    extends State<ChooseRelatedCompanyWidget> {
  _ChooseRelatedCompanyWidgetState(
      this.selectedItemBloc, this.httpEvent, this.selected);
  ...
  void initState() {
    super.initState();
    DropDownGetRelatedCompanyEvent downGetRelatedCompanyEvent =
        DropDownGetRelatedCompanyEvent();
    httpBloc = downGetRelatedCompanyEvent.bloc;
    Future.delayed(Duration.zero, () {
      locator<Web>().post(downGetRelatedCompanyEvent, context);
    });
  }
  ...
  Widget getListItem(Company item) {
    return Padding(
      ...
      child: Material(
          ...
          child: InkWell(
              onTap: () {
                selected = DropDownItem(item.id, item.organization_name, null);
                if (selectedItemBloc != null)
                  selectedItemBloc.select(
                      DropDownItem(item.id, item.organization_name, null));
              ...
              if (snapshot.hasData) {
                ...
                if (list.length > 0) {
                  ...
                  return Align(
                    ...
                    child: MyText(
                      text: Strings
                          .BottomSheetWidget_ChooseRelatedCompany_not_exist,
                      textAlign: TextAlign.right,
