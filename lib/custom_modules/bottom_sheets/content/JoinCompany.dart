...
class JoinCompanyWidget extends StatefulWidget {
  JoinCompanyWidget({
    Key key,
  ...
  @override
  _JoinCompanyWidgetState createState() => _JoinCompanyWidgetState(
        this.companyId,
      );
}

class _JoinCompanyWidgetState extends State<JoinCompanyWidget> {
  _JoinCompanyWidgetState(this.companyId);
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      locator<Web>()
          .post(CompanyGetDetailsEvent(httpBloc, this.companyId), context);
    });
    sendRequestHttpBloc.stream.listen((event) {
      if (event == true) {
      ...
      children: [
        StreamBuilder(
            ...
            builder: (context, AsyncSnapshot<dynamic> snapshot) {
              if (snapshot.hasData) {
                var item = snapshot.data as Company;
                return Row(
            ...
            children: [
              ...
              Expanded(
                child: FractionallySizedBox(
                    ...
                    widthFactor: 1,
                    child: MyButton(
                        text: Strings.BottomSheetWidget_JoinCompany_send,
                        buttonFill: ButtonFillStyle.Yellow,
                        onPressed: () {
                          if( _formKey.currentState.validate()){
                            locator<Web>().post(
                                CompanyRequestMemberShipEvent(
                                    sendRequestHttpBloc, this.company),
                                context);
                          }
